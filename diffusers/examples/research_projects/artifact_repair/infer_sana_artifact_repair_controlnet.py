...
from sana_dof import decode_vae_latents, encode_vae_latents, tensor_stats  # noqa: E402
...
from train_sana_artifact_repair_controlnet import ControlConditionProjection  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser(description="Infer SANA artifact repair ControlNet.")
...
def load_config(checkpoint):
    ...
    raise FileNotFoundError(f"Missing artifact_repair_config.json in {checkpoint}")
...
def load_pipeline(checkpoint, model, dtype, options):
    ...
    projection = ControlConditionProjection(
        int(config.get("control_condition_channels", 6)),
...
def generate(
    ...
    for timestep in sliced_timesteps:
        ...
        expanded_control = controlnet_cond
        if expanded_control.shape[0] != latent_model_input.shape[0]:
            expanded_control = expanded_control.repeat(latent_model_input.shape[0], 1, 1, 1)
        control_samples = controlnet(
        ...
        noise_pred = pipe.transformer(
            ...
            encoder_attention_mask=prompt_mask,
            timestep=timestep_input,
            controlnet_block_samples=tuple(s.to(pipe.transformer.dtype) for s in control_samples),
            return_dict=False,
        )[0].float()
        if do_cfg:
    ...
    stats = {
        ...
        "init_latents": tensor_stats(src_latents),
    ...
    return image, latents, src_latents, stats
...
def main():
    ...
    canvas_w, canvas_h = size_info["canvas_size"]
    control_condition = make_condition(pipe, prepared, size_info, args.zero_src_condition, args.zero_ref_condition)
    generator = torch.Generator(device="cuda").manual_seed(args.seed)
    image, final_latents, src_latents, stats = generate(
        pipe,
        controlnet,
        projection,
        args.prompt,
    ...
    if args.debug_latent_dir:
        ...
        decode_latents_to_pil(pipe, src_latents).save(debug / "decoded_init_latents.png")
        ...
        (debug / "control_condition_stats.json").write_text(
            json.dumps(tensor_stats(control_condition), indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
