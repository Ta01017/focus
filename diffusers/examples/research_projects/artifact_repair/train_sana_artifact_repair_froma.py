...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SANA artifact repair from-src adapter without ControlNet.")
    ...
    parser.add_argument("--loss_mode", choices=("legacy_src_noise_gt_velocity", "standard_sft"), default="legacy_src_noise_gt_velocity")
    parser.add_argument("--adapter_condition_channels", type=int, default=6)
...
class SrcRefAdapter(nn.Module):
    def __init__(self, in_channels=6, latent_channels=32):
        super().__init__()
        ...
        print("initialized artifact repair from-src adapter: [src_rgb, ref_rgb] -> latent residual", flush=True)

    def forward(self, x):
        return self.proj(x)


class SanaArtifactRepairFromAModel(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states, encoder_attention_mask, timestep, src_ref_condition, adapter_scale=1.0):
        raw_shape = list(src_ref_condition.shape)
        if src_ref_condition.shape[-2:] != hidden_states.shape[-2:]:
            src_ref_condition = F.interpolate(
                src_ref_condition.float(),
                size=hidden_states.shape[-2:],
                mode="bilinear",
                align_corners=False,
            )
        downsampled_shape = list(src_ref_condition.shape)
        residual = self.adapter(src_ref_condition.float()).to(hidden_states)
        self.last_shape_stats = {
            "src_ref_condition_raw_shape": raw_shape,
            "src_ref_condition_downsampled_shape": downsampled_shape,
            "hidden_states_shape": list(hidden_states.shape),
            "adapter_residual_shape": list(residual.shape),
        }
        self.last_residual_stats = {
            "mean": float(residual.detach().float().mean().cpu()),
            "std": float(residual.detach().float().std().cpu()),
            "norm": float(residual.detach().float().norm().cpu()),
        }
        hidden_states = hidden_states + adapter_scale * residual.to(hidden_states)
...
def save_checkpoint(output_dir, global_step, unwrapped, pipe, args, final=False):
    ...
    save_file({k: v.detach().cpu() for k, v in unwrapped.adapter.state_dict().items()}, ckpt / "adapter.safetensors")
    ...
    config.update(
        {
            ...
            "model": args.model,
            "condition_channels": args.adapter_condition_channels,
            "latent_channels": pipe.transformer.config.in_channels,
            "default_strength": 0.15,
            "default_schedule_mode": "sliced",
            "global_step": global_step,
        }
    )
    write_json(ckpt / "adapter_config.json", config)
    write_json(ckpt / "artifact_repair_config.json", config)


def main():
    args = parse_args()
    if args.batch_size != 1:
        raise ValueError("Dynamic from-src training currently requires --batch_size 1.")
    if args.adapter_condition_channels != 6:
        raise ValueError("--adapter_condition_channels must be 6 for [src_rgb, ref_rgb].")
    ...
    adapter = SrcRefAdapter(args.adapter_condition_channels, pipe.transformer.config.in_channels)
    model = SanaArtifactRepairFromAModel(pipe.transformer, adapter)
    ...
    trainable_params = list(adapter.parameters()) + [p for p in pipe.transformer.parameters() if p.requires_grad]
    if not trainable_params:
        raise ValueError("optimizer got an empty parameter list: adapter and LoRA parameters are not trainable.")
    ...
    accelerator.print(f"adapter trainable params: {sum(p.numel() for p in adapter.parameters()):,}", flush=True)
    ...
    while global_step < args.max_train_steps:
        for batch in dataloader:
            with accelerator.accumulate(model):
                ...
                src_ref_condition = build_control_condition(batch["src"], batch["ref"]).to(accelerator.device)
                pred = model(noisy_latents.to(dtype), prompt_embeds, prompt_mask, timesteps, src_ref_condition, args.adapter_scale).float()
                ...
                grad = None
                for p in accelerator.unwrap_model(model).adapter.parameters():
                ...
                if accelerator.sync_gradients:
                    if grad is None or float(grad.detach().cpu()) == 0.0:
                        print("WARNING: adapter_grad_norm is None or zero.", flush=True)
            ...
            if accelerator.sync_gradients:
                ...
                zero_residual_steps = zero_residual_steps + 1 if residual_norm == 0 else 0
                if zero_residual_steps >= 3:
                    print("WARNING: adapter residual norm has been zero for multiple steps.", flush=True)
                if accelerator.is_main_process and (global_step <= 5 or global_step % args.log_steps == 0):
                    ...
                    print(json.dumps({
                        ...
                        "adapter_grad_norm": None if grad is None else float(grad.detach().cpu()),
                        "adapter_residual": unwrapped.last_residual_stats,
