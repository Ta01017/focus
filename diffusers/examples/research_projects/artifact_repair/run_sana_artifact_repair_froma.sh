...
OUTPUT_DIR=${OUTPUT_DIR:-${DATA_ROOT}/focus/models/train/sana_artifact_repair_froma}
...
ADAPTER_CONDITION_CHANNELS=${ADAPTER_CONDITION_CHANNELS:-6}
...
REF_IMAGE=${REF_IMAGE:-}
PROMPT=${PROMPT:-}
OUTPUT_PATH=${OUTPUT_PATH:-${OUTPUT_DIR}/artifact_repair_froma.png}
...
echo "[ADAPTER_CONDITION_CHANNELS] ${ADAPTER_CONDITION_CHANNELS}"
...
run_train() {
  ...
  accelerate launch --mixed_precision "${MIXED_PRECISION}" \
    "${SCRIPT_DIR}/train_sana_artifact_repair_froma.py" \
    --model "${MODEL}" \
    --dataset_metadata_path "${DATASET_METADATA_PATH}" \
    --dataset_base_path "${DATASET_BASE_PATH}" \
    --output_dir "${OUTPUT_DIR}" \
    --loss_mode "${LOSS_MODE}" \
    --adapter_condition_channels "${ADAPTER_CONDITION_CHANNELS}" \
...
run_infer() {
  ...
  if [[ "${ZERO_REF_CONDITION}" == "1" ]]; then
    extra_args+=(--zero_ref_condition)
  fi
  if [[ "${ZERO_SRC_CONDITION}" == "1" ]]; then
    extra_args+=(--zero_src_condition)
  fi
  if [[ -n "${DEBUG_LATENT_DIR}" ]]; then
    extra_args+=(--debug_latent_dir "${DEBUG_LATENT_DIR}")
  fi
  python "${SCRIPT_DIR}/infer_sana_artifact_repair_froma.py" \
...
run_batch_infer() {
  ...
  python "${SCRIPT_DIR}/batch_infer_sana_artifact_repair_froma.py" \
    --checkpoint "${CHECKPOINT}" \
    --model "${MODEL}" \
    --dataset_metadata_path "${DATASET_METADATA_PATH}" \
    --dataset_base_path "${DATASET_BASE_PATH}" \
