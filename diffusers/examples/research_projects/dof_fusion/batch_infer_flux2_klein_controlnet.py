...
def main():
    ...
    for index, record in enumerate(records):
        ...
        focus_tokens = focus_map_to_tokens(
            focus, latent_height, latent_width, torch.device("cuda")
        )
        generator = torch.Generator(device="cuda").manual_seed(int(record.get("seed", args.seed + index)))
        with transformer.use_focus_condition(focus_tokens):
            image = pipe(
                image=[image_a, image_b],
                prompt=record.get("prompt") or args.prompt,
                height=args.height,
                width=args.width,
