...
## SANA ControlNet AB Fusion：focus-aware repair 增强

近期实验显示，旧版 SANA ControlNet AB fusion 虽然能利用 A/B RGB condition，但没有充分利用
focus map 做区域路由：

- `zero_rgb` 明显变差，说明 A/B RGB condition 有效。
- `zero_focus` 与 normal 接近，说明旧版 focus 条件没有被充分使用。
- 高 `strength` 会破坏形状，不能靠全图提高 strength 解决模糊区域。
- b_ref 区域 Laplacian 统计显示，旧版输出高频接近 A，而不是 B/GT。

为此，`train_sana_controlnet_ab_fusion.py` 新增了默认关闭的 focus-aware 监督：

- `--use_focus_loss_weighting`：启用 `M_keep / M_bref / M_gen` 区域加权 flow loss。
- `--x0_loss_weight`：从 predicted velocity 估计 `x0_pred = noise - pred`，加入 latent reconstruction loss。
- `--keep_consistency_loss_weight`：在 A 清晰区域约束 `x0_pred` 接近 A latent 或 GT latent。
- `--pixel_loss_weight`：可选 pixel L1 loss，支持区域权重和 `pixel_loss_max_pixels` 控制显存。
- `--use_bref_latent_residual`：可选 masked B latent residual，只在 `M_bref` 区域注入 `B-A` latent residual。

推理端 `infer_sana_controlnet_ab_fusion.py` 和 `batch_infer_sana_controlnet_ab_fusion.py` 也支持：

```bash
--use_bref_latent_residual \
--bref_latent_residual_weight 0.3 \
...
1. baseline compatibility：不加任何新参数，确认旧命令行为不变。
2. focus loss weighting：开启 `--use_focus_loss_weighting`，观察日志中的 `keep_mean / bref_mean / gen_mean`。
3. focus loss + x0：增加 `--x0_loss_weight 0.1`。
4. focus loss + x0 + pixel：小分辨率或低频率开启 `--pixel_loss_weight`。
5. focus loss + x0 + B latent residual：增加 `--use_bref_latent_residual --bref_latent_residual_weight 0.2~0.5`。

开启 focus-aware loss 后，训练日志会打印 `loss_flow / loss_x0 / loss_keep / loss_pixel`、三类 mask
均值、`source_delta_mean`、`bref_latent_mean` 和 ControlNet residual 统计，用于判断 focus map
是否真正参与训练，以及 b_ref 区域面积和 residual 注入是否合理。
