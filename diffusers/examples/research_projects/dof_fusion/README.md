...
## SANA Focus Route1：latent injection LoRA

新增一条轻量路线：`sana_focus_latent_injection`。它不使用 ControlNet、不使用 image cross-attention、
不把 A/B 拼到 text condition，而是在 SANA image token stream 中加入一个 zero-init 条件残差。

两种模式共用同一套 Python：

- `condition_mode=a_only`：只使用 `edit_image[0] = A`。
- `condition_mode=ab`：使用 `edit_image[0] = A` 和 `edit_image[1] = B`，A/B 在 latent channel 维拼接。

focus map 本轮只读取和记录，不注入模型，不加 focus loss。
...
A-only: condition_latents = z_a
AB:     condition_latents = cat([z_a, z_b], channel)

target_tokens = SANA.patch_embed(z_t)
injected_tokens = FocusLatentConditionInjector(condition_latents)
hidden_tokens = target_tokens + condition_scale * injected_tokens
```

训练只优化：

- `FocusLatentConditionInjector`
- Transformer LoRA，默认开启

VAE、Text Encoder、SANA base Transformer 权重保持冻结。

### Checkpoint 结构

```text
checkpoint-N/
  focus_condition_injector.safetensors
  focus_latent_injection_config.json
  transformer_lora/
...
`focus_latent_injection_config.json` 会记录 `condition_mode`。A-only checkpoint 不能加载到 AB，AB checkpoint
也不能加载到 A-only。
...
LORA_SCOPE=wide \
CONDITION_SCALE=1.0 \
OUTPUT_DIR=/data/vjuicefs_ai_camera_3drg_ql/public_data/11193880/focus/models/train/sana_focus_latent_injection/a_only/tiny16_a_only_fp32_lr1e5_1mp_widelora_$(date +%Y%m%d_%H%M%S) \
bash examples/research_projects/dof_fusion/run_sana_focus_a_latent_injection_lora.sh
```
...
LORA_SCOPE=wide \
CONDITION_SCALE=1.0 \
OUTPUT_DIR=/data/vjuicefs_ai_camera_3drg_ql/public_data/11193880/focus/models/train/sana_focus_latent_injection/ab/tiny16_ab_fp32_lr1e5_1mp_widelora_$(date +%Y%m%d_%H%M%S) \
bash examples/research_projects/dof_fusion/run_sana_focus_ab_latent_injection_lora.sh
```
...
```bash
MODE=batch_infer \
CHECKPOINT=/path/to/a_only/checkpoint-5000 \
INFER_OUTPUT_DIR=/path/to/a_only/infer_s030 \
STRENGTH=0.30 \
DTYPE=fp32 \
STEPS=20 \
GUIDANCE_SCALE=1.0 \
...
```bash
MODE=batch_infer \
CHECKPOINT=/path/to/ab/checkpoint-5000 \
INFER_OUTPUT_DIR=/path/to/ab/infer_s030 \
STRENGTH=0.30 \
DTYPE=fp32 \
STEPS=20 \
GUIDANCE_SCALE=1.0 \
...
OUTPUT=/tmp/focus_route1_sanity.png \
bash examples/research_projects/dof_fusion/run_sana_focus_a_latent_injection_lora.sh
```
