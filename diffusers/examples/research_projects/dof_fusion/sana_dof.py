...
class ConditionedSanaTransformer(nn.Module):
    ...
    @contextmanager
    def use_condition(self, cond_a_latents: torch.Tensor, cond_b_latents: torch.Tensor):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Reserved for a future focus-aware adapter. They are consumed here so they are not forwarded to Diffusers.
        del focus_a, focus_b
        ...
        condition = self.adapter(cond_a_latents, cond_b_latents).to(
...
def encode_condition_images(pipe, image_a, image_b, height: int, width: int, device: torch.device):
    ...
    cond_a_latents = pipe.vae.encode(cond_a).latent * pipe.vae.config.scaling_factor
    cond_b_latents = pipe.vae.encode(cond_b).latent * pipe.vae.config.scaling_factor
