...
from conditioning import MultiImageConditionAdapter, encode_condition_batch
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Sana SFT output containing transformer LoRA and condition adapter files.",
    )
    parser.add_argument(
        "--sprint_base_model",
        type=str,
        default="Efficient-Large-Model/Sana_Sprint_0.6B_1024px_diffusers",
        help="Released Sprint pipeline used for scheduler and inference components.",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument("--data_file_keys", type=str, default="image,edit_image")
    parser.add_argument("--extra_inputs", type=str, default="edit_image")
...
class SanaTrigFlow(SanaTransformer2DModel):
    def __init__(self, original_model, condition_adapter, guidance=False):
    ...
    def state_dict(self, *args, **kwargs):
        ...
        return {
            ...
            if not key.startswith(("condition_adapter.", "logvar_linear."))
        }
    ...
    def forward(
        ...
        **kwargs,
        ...
        if condition_latents is None:
            raise ValueError("condition_latents are required for conditioned Sana-Sprint distillation.")
        ...
        condition_parameter = next(self.condition_adapter.parameters())
        condition_latents = condition_latents.to(condition_parameter)
        if condition_latents.shape[0] != latent_model_input.shape[0]:
            if latent_model_input.shape[0] % condition_latents.shape[0]:
                raise ValueError("Condition batch cannot be expanded to match the denoiser batch.")
            repeats = latent_model_input.shape[0] // condition_latents.shape[0]
            condition_latents = condition_latents.repeat(repeats, 1, 1, 1, 1)
        condition = self.condition_adapter(condition_latents)
        if condition.shape[-2:] != latent_model_input.shape[-2:]:
            condition = F.interpolate(condition, latent_model_input.shape[-2:], mode="bilinear", align_corners=False)
        conditioned_model_input = latent_model_input + condition.to(latent_model_input)
        ...
        if jvp and self.gradient_checkpointing:
            ...
            model_out = super().forward(
                hidden_states=conditioned_model_input,
...
def save_condition_adapter(model, output_dir, args, teacher_config, global_step=None):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    save_file(
        {
            name: tensor.detach().cpu().contiguous()
            for name, tensor in model.condition_adapter.state_dict().items()
        },
        output_dir / "condition_adapter.safetensors",
    )
    config = {
        "base_model": args.sprint_base_model,
        "teacher_model": args.pretrained_model_name_or_path,
        "pipeline_type": "sana_sprint",
        "target_key": args.data_file_keys.split(",")[0].strip(),
        "condition_key": args.extra_inputs.strip(),
        "prompt_key": args.prompt_key,
        "num_condition_images": teacher_config["num_condition_images"],
        "latent_channels": teacher_config["latent_channels"],
        "hidden_channels": teacher_config["hidden_channels"],
        "resolution": args.resolution,
        "distillation": "official_scm_ladd",
    }
...
def main(args):
    if args.validation_prompt is not None:
        raise ValueError("Conditioned validation requires reference images; use infer.py after saving a checkpoint.")
    ...
    teacher_adapter_path = Path(args.teacher_adapter_path)
    teacher_adapter_config = json.loads(
        (teacher_adapter_path / "condition_adapter.json").read_text(encoding="utf-8")
    )
    if teacher_adapter_config.get("pipeline_type") != "sana":
        raise ValueError("--teacher_adapter_path must point to a Sana SFT output, not a Sprint output.")
    ...
    lora_state_dict, lora_metadata = SanaPipeline.lora_state_dict(
        teacher_adapter_path, return_lora_metadata=True
    )
    for teacher_transformer in (ori_transformer, ori_transformer_no_guide):
        SanaPipeline.load_lora_into_transformer(
            lora_state_dict,
            transformer=teacher_transformer,
            metadata=lora_metadata,
        )
        teacher_transformer.fuse_lora(safe_fusing=True)
        teacher_transformer.unload_lora()

    original_model_file = Path(args.pretrained_model_name_or_path) / "transformer/diffusion_pytorch_model.safetensors"
    if not original_model_file.is_file():
        original_model_file = hf_hub_download(
            args.pretrained_model_name_or_path,
            filename="transformer/diffusion_pytorch_model.safetensors",
            revision=args.revision,
        )
    original_state_dict = load_file(original_model_file)
    ...
    teacher_adapter = MultiImageConditionAdapter(
        teacher_adapter_config["latent_channels"],
        teacher_adapter_config["num_condition_images"],
        teacher_adapter_config["hidden_channels"],
    )
    teacher_adapter.load_state_dict(
        load_file(teacher_adapter_path / "condition_adapter.safetensors"), strict=True
    )
    student_adapter = copy.deepcopy(teacher_adapter)

    transformer = SanaTrigFlow(ori_transformer, student_adapter, guidance=True).train()
    pretrained_model = SanaTrigFlow(ori_transformer_no_guide, teacher_adapter, guidance=False).eval()
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                student_model = unwrap_model(transformer)
                teacher_model = unwrap_model(pretrained_model)
                for model in models:
                    unwrapped_model = unwrap_model(model)
                    # Handle transformer model
                    if unwrapped_model is student_model:
                        model = unwrapped_model
                        model.save_pretrained(os.path.join(output_dir, "transformer"))
                        save_condition_adapter(model, output_dir, args, teacher_adapter_config, global_step)
                        save_file(
                            {
                                name: tensor.detach().cpu().contiguous()
                                for name, tensor in model.logvar_linear.state_dict().items()
                            },
                            os.path.join(output_dir, "logvar_linear.safetensors"),
                        )
                    # Handle discriminator model (only save heads)
                    elif unwrapped_model is teacher_model:
                        pass
        ...
        def load_model_hook(models, input_dir):
            ...
            if not accelerator.distributed_type == DistributedType.DEEPSPEED:
                while len(models) > 0:
                    ...
                    if unwrapped_model is student_model:
                        loaded = SanaTransformer2DModel.from_pretrained(input_dir, subfolder="transformer")
                        unwrapped_model.load_state_dict(loaded.state_dict(), strict=False)
                        unwrapped_model.condition_adapter.load_state_dict(
                            load_file(os.path.join(input_dir, "condition_adapter.safetensors")), strict=True
                        )
                        unwrapped_model.logvar_linear.load_state_dict(
                            load_file(os.path.join(input_dir, "logvar_linear.safetensors")), strict=True
                        )
    ...
    extra_inputs = [key.strip() for key in args.extra_inputs.split(",") if key.strip()]
    if len(data_file_keys) != 2 or extra_inputs != [data_file_keys[1]]:
        raise ValueError("Expected --data_file_keys 'TARGET,CONDITION' and --extra_inputs 'CONDITION'.")
    target_key, condition_key = data_file_keys
    train_dataset = DiffSynthMetadataDataset(
    ...
    if train_dataset.num_condition_images != teacher_adapter_config["num_condition_images"]:
        raise ValueError(
            "Dataset condition count does not match the trained Sana teacher: "
            f"{train_dataset.num_condition_images} != {teacher_adapter_config['num_condition_images']}."
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_values = batch["target"].to(accelerator.device, dtype=vae.dtype)
            ...
            with torch.no_grad():
                condition_latents = encode_condition_batch(
                    vae, batch["conditions"].to(accelerator.device, dtype=vae.dtype)
                ...
                with accelerator.accumulate(models_to_accumulate):
                    with torch.no_grad():
                        ...
                        if args.train_largest_timestep:
                            ...
                            t = torch.where(random_mask, torch.full_like(t, args.largest_timestep), t)
                    ...
                    disc_fake_conditions = condition_latents
                    disc_fake_prompt_embeds = prompt_embeds
                    disc_fake_prompt_attention_mask = prompt_attention_mask
                    ...
                    if args.misaligned_pairs_D and bsz > 1:
                        ...
                        disc_fake_prompt_embeds = torch.cat([prompt_embeds, prompt_embeds], dim=0)
                        disc_fake_prompt_attention_mask = torch.cat(
                            [prompt_attention_mask, prompt_attention_mask], dim=0
                        )
                        disc_fake_conditions = torch.cat([condition_latents, condition_latents], dim=0)
                    ...
                    pred_fake = disc(
                        ...
                        encoder_hidden_states=disc_fake_prompt_embeds,
                        encoder_attention_mask=disc_fake_prompt_attention_mask,
                        condition_latents=disc_fake_conditions,
                    )
                    pred_true = disc(
    ...
    if accelerator.is_main_process:
        ...
        save_condition_adapter(transformer, args.output_dir, args, teacher_adapter_config, global_step)
        ...
        if args.push_to_hub:
            save_model_card(
                ...
                instance_prompt=args.default_prompt,
